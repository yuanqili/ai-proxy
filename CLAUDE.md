...
- **LiteLLM pricing catalog refresh** (`pricing/catalog.py` + `pricing/refresher.py`). Rather than manually curate `pricing_seed.json` when a new model lands, the Pricing tab has a "Refresh from LiteLLM catalog" button that pulls `model_prices_and_context_window.json` from the upstream BerriAI/litellm repo, normalizes the 2600+ entries down to our three providers (OpenAI / Anthropic / OpenRouter chat models only — Azure, Bedrock, fine-tunes, embeddings, image gen all filtered out), and diff-previews the changes (would_add / would_supersede / unchanged) before writing. OpenRouter entries have the `openrouter/` key prefix stripped so the model matches what clients send in `body.model`. Apply uses the existing `upsert_current`, which closes the prior effective row and inserts a new one — full versioned history is preserved. **Auto-refresh:** a background task in `lifespan` (`pricing.refresher.periodic_refresh`) runs the same apply logic every 24h after a 60s startup delay. `apply_catalog` skips unchanged rows, so the daily tick is a no-op unless LiteLLM actually shipped a price change.
...
|---|---|---|
| `ai.sat1600ap5.online` | `002-bollegecoard.caddy` | `aiproxy:8000` (this app) — `/metrics*` returns 404 externally; Prometheus scrapes via the internal `caddy-net` alias |
| `grafana.sat1600ap5.online` | `003-logs-stack.caddy` | `grafana:3000` |
| `promtail.sat1600ap5.online` | `003-logs-stack.caddy` | `promtail:9080` |
| `sat1600ap5.online` (root) | `001-bollegecoard-old.caddy.disabled` | Disabled — file renamed to `.disabled` so Caddy no longer serves the root hostname. Old `/home/ubuntu/stacks/openai-proxy/` is archived at `/home/ubuntu/archives/`. |
